...
2. Vous définissez ensuite la fonction `CounterReducer` avec deux paramètres : `state` et `action`.
 `state` est l'état actuel (qui est `INITIAL_STATE` par défaut si aucun état n'est passé), et `action` est l'objet envoyé depuis votre composant ou ailleurs dans votre application.
