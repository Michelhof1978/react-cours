...
1. Vous définissez un objet `INITIAL_STATE` avec une propriété `cart` initialisée à `60`. 
Ceci sera l'état initial de la partie du magasin Redux dont `AddCartReducer` est responsable.

2. La fonction `AddCartReducer` est définie avec deux paramètres : `state` et `action`. `state` 
est l'état actuel (qui par défaut est `INITIAL_STATE` si aucun état n'est passé) et `action` est l'objet qui est dispatché depuis votre composant ou ailleurs dans votre application.
