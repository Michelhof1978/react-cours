...
5. `addToCartFunc` est une fonction qui dispatche une action de type `ADDCART`, avec un payload
 `cartData`.

6. Votre composant renvoie une div qui affiche les valeurs actuelles de `cart` et `count`, 
un champ de saisie pour définir la valeur de `cartData`, et un bouton qui, lorsqu'on clique dessus, 
dispatche l'action `ADDCART` en utilisant `addToCartFunc`.

Notez que ce composant suppose que vous avez configuré votre store et vos reducers Redux de manière 
appropriée pour gérer les actions `INCR`, `DECR` et `ADDCART`. Assurez-vous que vos reducers sont 
configurés pour gérer ces actions et mettre à jour l'état de manière appropriée. De plus, assurez-vous que votre store Redux est correctement fourni à un niveau supérieur dans votre arborescence de composants en utilisant le composant `Provider` de `react-redux`.
