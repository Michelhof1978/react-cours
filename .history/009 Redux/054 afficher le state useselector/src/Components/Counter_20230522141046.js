...
Veuillez noter que `State.Count 'suppose que` Count` est directement sur l'objet Redux State.
 ...
 différentes propriétés pour différentes tranches de l'état. Par exemple, si vous avez un état 
 comme `{compteur: {count: 0}}`, alors votre sélecteur devrait être `state => state.counter.count` pour sélectionner correctement le nombre.