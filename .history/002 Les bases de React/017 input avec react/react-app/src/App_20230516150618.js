...
L'expression `e.target.value` est utilisée pour accéder à la valeur actuelle d'un élément HTML dans un événement JavaScript.
Voici ce que chaque partie de `e.target.value` signifie :

- `e` : C'est généralement la variable utilisée pour représenter l'objet Event qui contient des informations sur l'événement déclenché. Dans le contexte de cet exemple, `e` fait référence à l'objet Event de l'événement "input".
- `target` : C'est une propriété de l'objet Event qui représente l'élément HTML sur lequel l'événement s'est produit. Dans ce cas, `target` fait référence à l'élément `<input>` sur lequel l'utilisateur saisit ou modifie la valeur.
- `value` : C'est une propriété de l'élément HTML qui représente la valeur actuelle de cet élément. Dans le contexte du champ de saisie `<input>`, `value` représente la valeur textuelle saisie ou modifiée par l'utilisateur.

Ainsi, `e.target.value` combine ces éléments pour accéder à la valeur actuelle du champ de saisie lors de l'événement "input". Cela permet de récupérer la valeur saisie par l'utilisateur et de l'utiliser dans le code JavaScript, par exemple pour la stocker dans un état, effectuer des calculs ou effectuer des actions spécifiques en fonction de cette valeur.