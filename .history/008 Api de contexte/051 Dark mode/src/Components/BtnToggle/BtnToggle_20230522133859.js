...
vous avez créé dans un autre fichier. Vous allez utiliser ce contexte pour accéder à l'état du
 thème et à la fonction pour basculer entre les thèmes.

4. `const {toggleTheme, theme} = useContext(ThemeContext)`: Vous utilisez l'hook `useContext` pour 
accéder aux valeurs passées dans le `ThemeContext`. En particulier, vous obtenez la fonction 
`toggleTheme` et la valeur actuelle du `theme`.
...
Si le `theme` est vrai (c'est-à-dire que le thème sombre est activé), le bouton aura la classe
 `btn-toggle dark-btn` et affichera "DARK". Si le `theme` est faux (c'est-à-dire que le thème clair est activé), le bouton aura la classe `btn-toggle` et affichera "LIGHT".