...
Le composant `Timer` est un autre composant React qui crée un compteur qui s'incrémente toutes les secondes. Il utilise également le hook `useEffect` pour définir certaines actions qui se produisent lorsque le composant est monté et démonté.
...
function Timer() {
  ...
  const [timer, setTimer] = useState(1)
```
Ici, l'état `timer` est initialisé à 1 avec l'aide du hook `useState`. `setTimer` est la fonction qui sera utilisée pour modifier la valeur de `timer`.
