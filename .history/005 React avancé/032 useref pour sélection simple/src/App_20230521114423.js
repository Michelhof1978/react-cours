...
Ce code est un exemple de composant React appelé `App` qui affiche trois vidéos et un bouton. Les vidéos sont contrôlées par des références (`ref`) React et un hook `useEffect`. 
...
3. `useEffect(() => {...}, []);` : Le hook `useEffect` exécute un effet de bord après chaque rendu. Dans cet effet, un `console.log` affiche la référence actuelle, puis un délai (`setTimeout`) est défini pour mettre en pause la vidéo après 3 secondes.
