...
function App() {
  ...
  const ref = useRef([]);//useRef = retourne un object séléctionné, utilisé surtout pour des animations
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      ref.current.pause();//La vidéo s arréterra au bout de 3 secondes
    }, 3000);
  }, []);
...
// Le code utilise des références pour stocker les éléments vidéo dans un tableau, puis utilise
//  `useEffect` pour accéder à ces éléments vidéo et les contrôler, en mettant en pause chaque vidéo
//   après un délai de 3 secondes. Le bouton "Toggle" ne semble pas avoir d'effet sur les vidéos dans
//    ce code tel qu'il est actuellement.