...
function App() {
  ...
  return (
    <div className="App">
        {dataImg && 
        <img src={dataImg}/>//ous vérifions si dataImg est défini grâce à {dataImg && ...}. Si dataImg est défini, alors nous affichons une balise img avec l'URL de l'image de chat comme source (src={dataImg}).
        alt="cat image"
        style={{width: "500px"}}
        />}
    </div>
...
// 3. `fetch('https://api.thecatapi.com/v1/images/search')`: Ici, nous utilisons la fonction `fetch` 
// pour faire une requête à l'API qui nous donne une image de chat aléatoire.

// 4. `.then(response => { return response.json(); })`: Après avoir reçu la réponse de l'API, nous la 
// convertissons en format JSON, car c'est le format que l'API nous renvoie.

// 5. `.then(data => { setDataImg(data[0].url) })`: Une fois que la réponse est convertie en JSON, nous
//  prenons l'URL de l'image de chat (qui est stockée à `data[0].url`) et nous l'attribuons à notre variable d'état `dataImg` en utilisant la fonction `setDataImg`.

// 6. Ensuite, dans la partie de rendu du composant (la fonction `return`), nous vérifions si `dataImg`
//  est défini grâce à `{dataImg && ...}`. Si `dataImg` est défini, alors nous affichons une balise `img` avec l'URL de l'image de chat comme source (`src={dataImg}`).

// En résumé, ce code affiche une image de chat aléatoire à partir de l'API The Cat API chaque fois que
