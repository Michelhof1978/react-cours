...
1. `const [dataImg, setDataImg] = useState();`: On utilise ici le Hook `useState` de React pour créer une variable d'état `dataImg`. Cette variable stocke l'URL de l'image de chat que nous récupérons de l'API. La fonction `setDataImg` est utilisée pour mettre à jour cette variable.

2. `useEffect(() => { ... }, [])`: Le Hook `useEffect` est utilisé pour exécuter le code dans la fonction dès que le composant est monté pour la première fois. C'est parce que nous avons passé un tableau vide `[]` comme second argument à `useEffect`, ce qui signifie que le code ne sera exécuté qu'au montage initial du composant, et pas lors des mises à jour ultérieures.
