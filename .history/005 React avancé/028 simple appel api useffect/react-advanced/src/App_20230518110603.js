...
function App() {
  ...
  useEffect(() => {
    ...
    .then(response => {//Alors si succés, on reçoit une réponse
      console.log(response);//Si réponse reçu, on reçoit une requêtte http
      return response.json();//On reçoit la reponse sous format json, va retourner un tableau avec un object dedans
    })
    .then(data => {
      console.log(data);
