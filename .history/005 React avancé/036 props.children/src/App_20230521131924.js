...
La fonction `toggleFunc` est définie pour modifier cet état. Elle convertit d'abord l'état actuel en
 un tableau avec l'opérateur de décomposition (spread operator). Ensuite, elle ajoute le chiffre 4 à ce tableau avec la méthode `push`, puis elle utilise `setToggle` pour mettre à jour l'état `toggle` avec ce nouveau tableau. Cependant, il y a un problème ici : vous ne pouvez pas décomposer une chaîne en un tableau puis y ajouter un chiffre. Cela causera une erreur.
