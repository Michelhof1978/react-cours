...
1. **useState** : Ce hook est utilisé pour définir un état local dans le composant. Ici, vous avez un état `toggle` qui est un tableau `[1,2,3]`. La fonction `setToggle` est utilisée pour modifier cet état.

2. **useMemo** : Ce hook est utilisé pour optimiser les performances en mémorisant les valeurs calculées coûteuses. Ici, vous créez un tableau `tableau` qui est une version mémorisée de `toggle`. Cependant, comme vous n'avez pas inclus `toggle` dans le tableau des dépendances de `useMemo`, le tableau ne sera jamais recalculé même lorsque `toggle` change. Pour résoudre ce problème, vous devriez modifier votre code pour inclure `toggle` dans le tableau des dépendances :
