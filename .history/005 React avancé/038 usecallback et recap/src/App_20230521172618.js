...
calculées coûteuses. Ici, vous créez un tableau `tableau` qui est une version mémorisée de `toggle`.
 Cependant, comme vous n'avez pas inclus `toggle` dans le tableau des dépendances de `useMemo`, le tableau ne sera jamais recalculé même lorsque `toggle` change. Pour résoudre ce problème, vous devriez modifier votre code pour inclure `toggle` dans le tableau des dépendances :
