...
La fonction `toggleFunc` est utilisée pour ajouter le nombre 4 au tableau chaque fois qu'elle est
 appelée. Elle crée d'abord une copie du tableau existant en utilisant l'opérateur de décomposition (spread operator), puis elle utilise la méthode `push` pour ajouter le nombre 4 à la copie, et enfin elle utilise `setToggle` pour mettre à jour l'état `toggle` avec le nouveau tableau.
