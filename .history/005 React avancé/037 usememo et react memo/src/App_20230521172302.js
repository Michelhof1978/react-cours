...
function App() {
  ...
  const tableau = useMemo(() => {//A utiliser uniquement pour les tableau ou objects
    return [1,2,3,4]
...
// changent. Il prend deux arguments : une fonction de création et une liste de dépendances. 
// La fonction de création est exécutée lorsque le hook est rendu pour la première fois et chaque fois
//  que les dépendances changent. La valeur qu'elle retourne est mémorisée et renvoyée lors des rendus
//   ultérieurs tant que les dépendances restent les mêmes.
...
// Dans cet exemple, le calcul coûteux dans `useMemo` n'est effectué que lorsque `a` ou `b` changent. 
// Si ces props ne changent pas entre les rendus, `useMemo` renvoie simplement la valeur mémorisée au
//  lieu d'exécuter à nouveau le calcul.

// Notez que `useMemo` ne devrait être utilisé que lorsque le calcul de la valeur est vraiment coûteux.
//  Les hooks de React ajoutent un certain coût, il est donc important de ne pas les utiliser 
//  inutilement. En général, vous devriez éviter d'optimiser prématurément votre application.
//   Commencez par rendre votre code propre et facile à comprendre, puis optimisez seulement les parties
//    qui sont clairement lentes après avoir profilé votre application.