...
function App() {
  ...
  useEffect(() => {
    ...
    const intervalID = setInterval(() => {//Permet de rafraichir le state en un nouveau state chaque seconde, setInterval va retourner un id
    ...
    return () => {//Ici, on doit pouvoir effacer la mémoire en supprimant le id de intervalID quand le composant sinon, la mémoire sera encombré
