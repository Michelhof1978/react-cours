...
function App() {
  ...
  useEffect(() => {
    ...
    return () => {//Sera executé lorsque le composant sera détruit
      ...
      Dans cet exemple spécifique, elle supprime la fonction actionResize qui a été précédemment 
      ajoutée comme un gestionnaire d'événement pour l'événement 'resize' de l'objet window.

L'événement 'resize' se déclenche chaque fois que la fenêtre du navigateur est redimensionnée.
Si vous n'enlevez pas cet événement lorsque le composant est démonté, cela pourrait causer des 
fuites de mémoire, car la fonction actionResize continuerait à s'exécuter en arrière-plan même si le
composant n'est plus présent dans le DOM.
C'est pourquoi cette ligne est souvent utilisée dans la fonction de nettoyage retournée par 
useEffect. Cette fonction est exécutée avant que le composant ne soit démonté, ce qui donne l'opportunité de nettoyer tout ce qui a été mis en place lors de l'exécution de useEffect. Dans ce cas, elle est utilisée pour supprimer le gestionnaire d'événement 'resize' pour éviter des problèmes potentiels.
