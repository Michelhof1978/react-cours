  ...
  return (
    <div className="App">
        {dataArr.map((item, index) => {// map va retourner un nouveau tableau ,item et index sont des paramètres de la fonction de rappel utilisée dans la méthode map()
          console.log(index);//Index permettra d afficher une index avec un ID unique sur chaque item
          return <p key={index}>Nom : {item.nom}</p>//key={index} est utilisé pour fournir une clé unique à chaque élément de la liste.
          //key permet de référencer chaque index avec un ID unique
       })}
    </div>
  );
}
