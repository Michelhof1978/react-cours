...
---------------------------------------------
                                       COMPOSANTS STATEFUL ET STATELESS
...
Installer react-redux: npm install react-redux (pour lier redux à react)

Redux est une bibliothèque JavaScript open source qui aide à gérer l'état de votre application. Il est couramment utilisé avec des bibliothèques comme React ou Angular pour construire des interfaces utilisateur. Voici une explication de ses concepts clés :

1. **Store**: Le store est l'objet qui détient l'état global de l'application. Il y a généralement un seul store dans une application Redux.

2. **Actions**: Les actions sont des objets qui contiennent une description de ce qui doit changer. Elles sont la seule façon de transmettre des données depuis votre application au store. Elles doivent avoir un champ `type` qui indique le type d'action à effectuer.

3. **Reducers**: Les reducers sont des fonctions qui spécifient comment l'état de l'application change en réponse à une action. En d'autres termes, elles décrivent la logique de modification de l'état. Pour chaque action dispatchée, le reducer produit le prochain état en fonction de l'état actuel et de l'action reçue.

Voici comment ils fonctionnent ensemble :

- Lorsque vous dispatchez une action, Redux appelle le reducer avec l'état actuel et l'action dispatchée. Le reducer calcule le nouvel état en fonction du type d'action.

- Le store sauvegarde le nouvel état retourné par le reducer. Toute l'interface utilisateur qui est connectée à ce morceau de l'état se met à jour automatiquement.

- Les actions sont généralement dispatchées suite à des interactions utilisateur, comme des clics de boutons ou des saisies de texte.

L'un des principaux avantages de Redux est qu'il rend l'état de votre application prévisible et facile à comprendre. C'est parce que toutes les modifications de l'état suivent le même cycle : dispatch d'action -> traitement dans le reducer -> mise à jour de l'état dans le store. En raison de cette uniformité, il est plus facile de suivre les modifications de l'état, de déboguer les problèmes et d'implémenter des fonctionnalités comme l'annulation/répétition.
------------------------------------------------------------------------------------------------------------------------------
                                    INSTALLATION REACT ROUTER POUR LES ROUTES
npm install react-router-dom
VOIR DOSSIER PROGRAMMEZOO POUR LE SYSTEME DE ROUTAGE
------------------------------------------------------------------------------------------------------------------------------
